...
const [users, setUsers] = useState<User[]>([]);
  ...
  const fetchUsers = async () => {
    try {
      ...
      const mappedUsers = data.map((user: any) => ({
        ...
        currency: user.wallet?.currency || user.currency || 'NGN',
  ...
  const handleClearAccount = async () => {
    ...
    try {
      ...
      toast.error('Failed to clear account');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const toggleForce = async () => {
    if (!selectedUser || !userCodes) return;
    try {
      setCodesLoading(true);
      const next = !userCodes.forced;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">      
      ...
      <Dialog open={creditDebitOpen} onOpenChange={setCreditDebitOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <div>
              <Label>Amount ({selectedUser?.currency || 'NGN'})</Label>
              <Input
                type="number"
                placeholder="0.00"
      ...
      <Dialog open={clearAccountOpen} onOpenChange={setClearAccountOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Ban className="h-5 w-5 text-orange-600" />
              Reset User Account (Clear Balance & History)
            </DialogTitle>
            <DialogDescription>
              This will delete all transactions and set balance to zero for {selectedUser?.firstName} {selectedUser?.lastName}.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setClearAccountOpen(false)}>
              Cancel
            </Button>
            <Button onClick={handleClearAccount} disabled={processing} className="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700">
              {processing ? 'Resetting...' : 'Reset Account'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold text-blue-600">{formatCurrency(selectedUser.balance || 0, selectedUser.currency)}</p>
                </div>
                <div>
                  <Label className="text-gray-500">Currency</Label>
                  <p className="font-semibold">{selectedUser.currency || 'NGN'}</p>
                </div>
                {selectedUser.dateOfBirth && (
                  <div>
                    <Label className="text-gray-500">Date of Birth</Label>
                    <p className="font-semibold">{new Date(selectedUser.dateOfBirth).toLocaleDateString()}</p>
                  </div>
                )}
