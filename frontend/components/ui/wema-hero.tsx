...
function SlideVisual({ type, icon }: { type: string; icon: string }) {
    if (type === "banking") {
        return (
            <div className="relative bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] shadow-2xl border border-white/50 p-2 sm:p-4 overflow-hidden w-full max-w-[260px] sm:max-w-[340px] md:max-w-none mx-auto">
                {/* Banking Dashboard */}
                <div className="bg-gradient-to-r from-brand-primary to-brand-secondary rounded-t-xl sm:rounded-t-[2rem] p-3 sm:p-6 text-white">
                    <div className="flex justify-between items-start mb-2 sm:mb-6">
                        ...
                        <div className="w-8 h-8 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-white/20 backdrop-blur flex items-center justify-center">
                            <span className="text-base sm:text-3xl">{icon}</span>
                        </div>
                    </div>
                    <div className="flex gap-2 sm:gap-4">
                        <div className="flex-1 bg-white/20 backdrop-blur rounded-lg sm:rounded-xl p-2 sm:p-3 text-center">
                ...
                <div className="p-3 sm:p-6">
                    <div className="grid grid-cols-4 gap-1 sm:gap-3">
                        {['Send', 'Pay', 'Cards', 'More'].map((action, i) => (
                            <div key={action} className="bg-slate-50 rounded-lg sm:rounded-2xl p-1.5 sm:p-4 text-center">
        ...
        return (
            <div className="relative w-full max-w-[340px] sm:max-w-none mx-auto">
                ...
                <div className="flex items-center justify-center gap-2 sm:gap-8">
                    {/* Left Phone - Sending */}
                    <div className="bg-white rounded-xl sm:rounded-3xl shadow-xl sm:shadow-2xl p-1.5 sm:p-3 transform sm:-rotate-6">
                        <div className="bg-brand-primary rounded-lg sm:rounded-2xl p-3 sm:p-6 text-white w-[100px] sm:w-48">
                            ...
                            <div className="bg-white/20 rounded-lg sm:rounded-xl p-1.5 sm:p-3 text-center">
                    ...
                    <div className="bg-white rounded-xl sm:rounded-3xl shadow-xl sm:shadow-2xl p-1.5 sm:p-3 transform sm:rotate-6">
                        <div className="bg-green-500 rounded-lg sm:rounded-2xl p-3 sm:p-6 text-white w-[100px] sm:w-48">
                ...
                {/* Speed indicator */}
                <div className="mt-4 sm:mt-8 bg-white/80 backdrop-blur rounded-xl sm:rounded-2xl shadow-xl p-2.5 sm:p-4 max-w-[220px] sm:max-w-xs mx-auto">
                    <div className="flex items-center gap-2 sm:gap-3">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-brand-primary flex items-center justify-center text-white">
                            <Send size={16} className="sm:w-5 sm:h-5" />
                        </div>
                        <div>
                            <div className="font-bold text-slate-900 text-sm sm:text-base">Instant Transfer</div>
                            <div className="text-xs sm:text-sm text-slate-500">Completed in 0.3s</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (type === "savings") {
        return (
            <div className="relative bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] shadow-2xl border border-white/50 p-3 sm:p-6 overflow-hidden w-full max-w-[260px] sm:max-w-none mx-auto">
                {/* Savings Goal Card */}
                <div className="flex items-center justify-between mb-3 sm:mb-6">
                    <div>
                        <div className="text-[10px] sm:text-sm text-slate-500 mb-0.5 sm:mb-1">Savings Goal</div>
                        <div className="text-base sm:text-2xl font-black text-slate-900">Vacation Fund</div>
                    </div>
                    <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center text-xl sm:text-3xl shadow-lg">
                        🎯
                    </div>
                </div>
                ...
                <div className="mb-3 sm:mb-6">
                    ...
                    <div className="text-right text-[10px] sm:text-sm text-slate-500 mt-0.5 sm:mt-1">75% Complete</div>
                </div>

                {/* Interest Rate */}
                <div className="bg-green-50 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 border border-green-100 mb-2 sm:mb-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div>
                                ...
                                <div className="text-[10px] sm:text-sm text-slate-500">Annual Yield</div>
                            </div>
                        </div>
    ...
    if (type === "business") {
        return (
            <div className="relative bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] shadow-2xl border border-white/50 p-2 sm:p-4 overflow-hidden w-full max-w-[260px] sm:max-w-none mx-auto">
                {/* Business Dashboard */}
                <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-t-xl sm:rounded-t-[2rem] p-3 sm:p-6 text-white">
                    ...
                    <div className="flex items-end gap-1 sm:gap-2 h-10 sm:h-16">
                        {[40, 65, 45, 80, 60, 90, 75].map((height, i) => (
                            <motion.div
                                ...
                                className="flex-1 bg-gradient-to-t from-brand-primary to-brand-secondary rounded-t-sm sm:rounded-t-lg"
                            />
                        ))}
                    </div>
                </div>
                ...
                <div className="p-3 sm:p-6">
                    ...
                    <div className="flex items-center justify-between mb-2 sm:mb-4">
                        ...
                        <div className="flex -space-x-1.5 sm:-space-x-2">
                            {['JD', 'SM', '+2'].map((initials, i) => (
                                <div
                                    key={i}
                                    className={`w-5 h-5 sm:w-8 sm:h-8 rounded-full ${i === 2 ? 'bg-slate-200 text-slate-600' : 'bg-brand-primary text-white'} flex items-center justify-center text-[8px] sm:text-xs font-bold border-2 border-white`}
                                >
                                    {initials}
                                </div>
                            ))}
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                        <div className="bg-slate-50 rounded-lg sm:rounded-xl p-2 sm:p-3 text-center">
                            ...
                            <div className="text-[9px] sm:text-xs text-slate-500">Invoices</div>
                        </div>
                        <div className="bg-green-50 rounded-lg sm:rounded-xl p-2 sm:p-3 text-center">
                            ...
                            <div className="text-[9px] sm:text-xs text-slate-500">Revenue</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 sm:px-6 relative z-10">
                <div className="grid lg:grid-cols-2 gap-4 lg:gap-12 xl:gap-20 items-center">
                    ...
                    <div className="text-center lg:text-left">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-brand-primary/10 rounded-full mb-4 sm:mb-8 border border-brand-primary/20">
                                    <Sparkles size={16} className="text-brand-primary" />
                                    <span className="text-sm font-semibold text-brand-primary">{slide.badge}</span>
                                </div>

                                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-black text-slate-900 leading-[1.1] mb-4 sm:mb-8 tracking-tight">
                                    {slide.headline}
                                    <span className="block mt-1 sm:mt-2 text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-secondary">
                                        {slide.highlight}
                                    </span>
                                </h1>

                                <p className="text-base sm:text-lg md:text-xl text-slate-600 mb-4 sm:mb-10 max-w-xl mx-auto lg:mx-0 leading-relaxed font-medium">
                                    {slide.description}
                                </p>
                            </motion.div>
                        </AnimatePresence>
                ...
                <div className="lg:hidden mt-6">
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mb-6">
                        ...
                        <Link href="/about">
                            <Button
                                size="lg"
                                variant="outline"
                                className="w-full sm:w-auto h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base font-bold rounded-xl border-2 border-slate-200 text-slate-700 hover:bg-white hover:border-brand-primary hover:text-brand-primary transition-all bg-white/50"
                            >
                                Learn More
                            </Button>
                        </Link>
                    ...
                    <div className="flex items-center gap-3 justify-center">
                        <button
                            onClick={prevSlide}
                            className="w-9 h-9 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-brand-primary hover:text-white transition-all"
                        >
                            <ChevronLeft size={16} />
                        </button>
                        <div className="flex gap-2">
