...
const _settingsservice = require("../../modules/settings/settings.service");
...
let EmailService = class EmailService {
    ...
    async sendKycStatusEmail(params) {
        ...
        const html = this.renderTemplate(brand, {
            ...
            lines: [
                status === 'APPROVED' ? 'You now have full access to all features.' : '',
    ...
    `;
        const html = this.renderTemplate(brand, {
            ...
            cta: {
                label: 'Verify Email',
                url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/user/profile`
            }
        });
        this.logSend(email, subject, html);
    }
    ...
    async sendTransferCodeEmail(params) {
    ...
    async sendSupportTicketEmail(params) {
    ...
    `;
        const html = this.renderTemplate(brand, {
            ...
            cta: {
                ...
                url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/user/support`
            }
        });
        this.logSend(email, subject, html);
    }
        ...
        const html = this.renderTemplate(brand, {
          ...
          <div style="background:#ffffff;border:1px solid #cbd5e1;padding:12px;border-radius:8px;font-family:monospace;word-break:break-all;font-size:13px">${cryptoWalletAddress}</div>
            ...
            cta: {
                ...
                url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/user/loans`
            }
        });
        this.logSend(email, subject, html);
    }
    ...
    async sendLoanDisbursementEmail(params) {
        ...
        const html = this.renderTemplate(brand, {
          ...
          <div style="color:#15803d;font-size:28px;font-weight:800">${currency} ${loanAmount}</div>
            ...
            cta: {
                label: 'View Wallet',
                url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/user/dashboard`
            }
        });
        this.logSend(email, subject, html);
    }
    async sendTransactionEmail(params) {
        ...
        ].includes(transactionType) || (description || '').toLowerCase().includes('from');
    ...
    `.trim();
        ...
        const html = this.renderTemplate(brand, {
            ...
            cta: {
                ...
                url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/user/transactions`
            }
        });
        this.logSend(email, subject, html);
    }
    async logSend(to, subject, html) {
        // Attempt real send via SMTP if configured; otherwise log
        try {
            const brandSettings = await this.settingsService.getSettings();
            const emailCfg = brandSettings.email || {};
    ...
    renderTemplate(brand, opts) {
        ...
        const introHtml = intro.includes('<br>') || intro.includes('<strong>') ? `<div style=\"margin:0 0 20px;color:#475569;line-height:1.7;font-size:15px\" class=\"dark-text-secondary\">${intro}</div>` : `<p style=\"margin:0 0 20px;color:#475569;line-height:1.6;font-size:15px\" class=\"dark-text-secondary\">${intro}</p>`;
        const button = cta ? `<table role=\"presentation\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\" style=\"margin:28px auto 0\"><tr><td align=\"center\" style=\"border-radius:12px;background:linear-gradient(135deg, ${primary} 0%, ${secondary} 100%);box-shadow:0 10px 25px -5px ${primary}40, 0 8px 10px -6px ${primary}30;transition:all 0.3s ease\"><a href=\"${cta.url}\" target=\"_blank\" style=\"display:inline-block;padding:16px 40px;color:#ffffff;text-decoration:none;font-weight:700;font-size:15px;line-height:1;border-radius:12px;letter-spacing:0.02em\">${cta.label}</a></td></tr></table>` : '';
        const support = brand.supportEmail ? `<p style=\"margin:0 0 8px;color:#64748b;font-size:13px;line-height:1.5\">Questions? Contact us at <a href=\"mailto:${brand.supportEmail}\" style=\"color:${primary};text-decoration:none;font-weight:500\">${brand.supportEmail}</a></p>` : '';
        const logoBlock = brand.logo ? `<img src=\"${brand.logo}\" alt=\"${brand.name}\" width=\"56\" height=\"56\" style=\"display:inline-block;vertical-align:middle;border-radius:12px;filter:brightness(0) invert(1);opacity:0.95\"/>` : `<div style=\"width:56px;height:56px;border-radius:12px;background:rgba(255,255,255,0.2);display:inline-block;vertical-align:middle\"></div>`;
