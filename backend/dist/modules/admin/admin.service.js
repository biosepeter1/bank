...
let AdminService = class AdminService {
    async getDashboardStats() {
        ...
        const activeUsers = await this.prisma.user.count({
            where: {
                role: 'USER',
                accountStatus: _client.AccountStatus.ACTIVE
            }
        });
        ...
        const totalTransactions = await this.prisma.transaction.count({
            where: {
                ...
                type: {
                    not: 'ADJUSTMENT'
                } // Exclude admin adjustments and initial deposits
            }
        });
        ...
        const largePendingTransactions = await this.prisma.transaction.count({
            where: {
                status: _client.TransactionStatus.PENDING,
                amount: {
                    gte: 1000000
    ...
    async getSystemAlerts() {
        ...
        const suspendedCount = await this.prisma.user.count({
            where: {
                accountStatus: _client.AccountStatus.SUSPENDED,
                role: 'USER'
            }
        });
        if (suspendedCount > 0) {
    ...
    async createUser(createUserDto, adminId) {
        ...
        if (existingUser) {
            throw new _common.BadRequestException('User with this email or phone already exists');
        }
        // Hash password
        ...
        const user = await this.prisma.$transaction(async (tx)=>{
            ...
            const newUser = await tx.user.create({
                data: {
                    ...
                    accountStatus: createUserDto.accountStatus || _client.AccountStatus.ACTIVE,
            ...
            await tx.kYC.create({
                data: {
                    userId: newUser.id,
                    status: createUserDto.kycStatus || _client.KYCStatus.PENDING
                }
            });
            // Create transfer codes (COT, IMF, TAX) - inactive by default
            const transferCodeTypes = [
                'COT',
                'IMF',
                'TAX'
            ];
    ...
    async updateUser(userId, updateUserDto, adminId) {
        ...
        if (updateUserDto.email || updateUserDto.phone) {
            ...
            if (conflict) {
                throw new _common.BadRequestException('Email or phone already in use');
            }
        }
    ...
    async updateUserBalance(userId, updateBalanceDto, adminId) {
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId
            },
        ...
        if (newBalance < 0) {
            throw new _common.BadRequestException('Insufficient balance for this adjustment');
        }
    ...
    async updateKycStatus(userId, updateKycDto, adminId) {
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId
            },
        ...
        if (!user.kyc) {
            throw new _common.NotFoundException('KYC record not found');
        }
        ...
        // Create audit log
        const action = updateKycDto.status === _client.KYCStatus.APPROVED ? 'KYC_APPROVED' : 'KYC_REJECTED';
        await this.prisma.auditLog.create({
            data: {
                userId: adminId,
                actorEmail: 'admin',
    ...
    async updateTransferCode(userId, codeType, updateCodeDto, adminId) {
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId
            }
        ...
        if (!transferCode) {
            throw new _common.NotFoundException('Transfer code not found');
        }
    ...
    async getSidebarCounts() {
        ...
        const pendingCardRequests = await this.prisma.cardRequest.count({
            where: {
                status: _client.CardRequestStatus.PENDING
            }
        }).catch(()=>0);
        // Count pending KYC
        const pendingKyc = await this.prisma.kYC.count({
